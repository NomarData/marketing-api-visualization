...
class PostProcessVisualizationData:
    ...
    def is_column_all_nan(self, column_name):
        return self.data[column_name].isnull().values.all()
    ...
    def generate_behavior_column(self):
        logging.info("Creating behavior column")
        if not self.is_column_all_nan(BEHAVIOR_column_name):
